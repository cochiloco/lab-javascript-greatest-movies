...
const noDuplicates = getAllDirectors.filter((item, index) => {
  console.log(`the item is: ${item}, the index is: ${index}`);
  if (getAllDirectors.indexOf(item) == index)
  //console.log(`the item is: ${item}, the index is: ${index}`);
  return item;
});
...
const allRates = movies.reduce((accumulator, item) => {
  return accumulator += item.rate;
},0)
let allRatesAverage = allRates / movies.length;
console.log(Math.floor(allRatesAverage));


// Iteration 4: Drama movies - Get the average of Drama Movies

const onlyDramaRates = movies.filter(movie =>{
  if (movie.genre == 'Drama'){
    return movie.rate;
  }
})

const onlyDramaAverageRate = (onlyDramaRates.reduce((acc,item)=>{
  return acc += item.rate;
},0))/onlyDramaRates.length;

console.log(onlyDramaAverageRate);
...
for(let i = 0; i<20; i++){
  console.log(orderAlphabetically[i].title);
}
